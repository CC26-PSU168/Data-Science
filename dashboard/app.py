...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**🎭 Filter Persona**")
    if "Persona" in df_all.columns:
        persona_options = sorted(df_all["Persona"].unique())
        selected_persona = st.multiselect(
            "Persona", persona_options, default=persona_options,
            label_visibility="collapsed"
        )
    else:
        selected_persona = None
        st.caption("⚠️ Kolom Persona belum ada di dataset.\nTambahkan di preprocessing dulu.")
...
# Apply persona filter if column exists
if selected_persona is not None and "Persona" in df.columns:
    df = df[df["Persona"].isin(selected_persona)]

df_exp = df[df["Transaction_Type"] == "EXPENSE"]
df_inc = df[df["Transaction_Type"] == "INCOME"]

# Guard: tampilkan pesan jika filter kosong
if df.empty or (df_exp.empty and df_inc.empty):
    st.warning("⚠️ Tidak ada data yang cocok dengan filter. Silakan ubah filter di sidebar.")
    st.stop()
...
avg_daily   = df_exp.groupby("Date")["Amount"].sum().mean() if not df_exp.empty else 0
...
with tab2:
    ...
    with c2:
        ...
        max_day = day_exp["Amount"].idxmax() if not day_exp.empty else None
...
with tab5:
    ...
    st.markdown("""
    <div style='background:linear-gradient(135deg,#0f2d1f,#1a3a2a);border:1px solid #1D9E75;
    border-radius:16px;padding:20px 24px;margin-bottom:24px;'>
        <div style='font-size:13px;color:#4ade80;font-weight:600;letter-spacing:.05em;
        text-transform:uppercase;margin-bottom:6px;'>Ringkasan Temuan</div>
        <div style='font-size:15px;color:#f1f5f9;line-height:1.7;'>
        Berdasarkan analisis <b>4.981 transaksi</b> dari 4 persona mahasiswa Indonesia
        (periode 2024–2025), ditemukan bahwa rata-rata mahasiswa menghabiskan
        <b style='color:#4ade80;'>Rp 1,5–1,6 juta/bulan</b> dengan pengeluaran
        terbesar pada kategori <b style='color:#4ade80;'>Makan & Minum</b> dan
        lonjakan di awal semester.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    BQS = [
        {
            ...
            "color": "#1D9E75",
            "finding": "Makan & Minum — Rp 70.464.313 dari 2.588 transaksi",
            "detail": "Rata-rata Rp 27.223/transaksi (setara 2–3x makan di warung per hari). "
                      "Tagihan menyusul dengan nominal per transaksi tertinggi (Rp 109.262).",
            "insight": "💡 Makan & Minum harus menjadi prioritas fitur budgeting karena frekuensi "
                       "tertinggi + dampak terbesar ke total pengeluaran bulanan."
        },
        {
            "no": "BQ2", "icon": "📅",
            "title": "Bulan Paling Boros",
            "color": "#3b82f6",
            "finding": "September — Rp 19.168.235 (tertinggi)",
            "detail": "Diikuti November (Rp 17,9jt) dan Juli (Rp 16,1jt). Pola ini berkorelasi "
                      "dengan awal semester baru. Juni paling rendah (akhir semester).",
            "insight": "💡 Model peringatan perlu lebih sensitif di Juli & September — bulan "
                       "dengan lonjakan pengeluaran yang dapat diprediksi."
        },
        {
            "no": "BQ3", "icon": "📆",
            "title": "Hari Paling Boros",
            "color": "#a78bfa",
            "finding": "Selasa — Rp 32.762.841 total pengeluaran",
            "detail": "Berlawanan dengan asumsi umum — hari kerja (Senin–Rabu) lebih boros dari "
                      "akhir pekan. Mahasiswa lebih aktif keluar di hari kuliah.",
            "insight": "💡 Fitur peringatan harian diprioritaskan Senin–Rabu sebagai hari "
        ...
        },
        {
            ...
            "color": "#f59e0b",
            "finding": "Rp 335.623/hari (rata-rata keseluruhan persona)",
            "detail": "Distribusi right-skewed: sebagian besar hari Rp 50rb–300rb, namun ada "
                      "outlier hingga Rp 4.079.506 (2 Jan 2024 — tagihan besar bersamaan).",
            "insight": "💡 Threshold peringatan realistis: Rp 150rb–200rb/hari (normal), "
                       "Rp 500rb+ (tidak biasa, perlu diperingatkan)."
        },
        {
            "no": "BQ5", "icon": "⚖️",
            "title": "Pemasukan vs Pengeluaran",
            "color": "#ef4444",
            "finding": "Defisit Rp 29.016.319 secara akumulatif",
            "detail": "Defisit ini tidak mencerminkan kondisi riil — income hanya 234 transaksi "
                      "vs 4.747 expense. Data asli (Persona 1) menunjukkan rata-rata "
                      "Rp 1.564.815/bulan, masih wajar untuk mahasiswa Indonesia.",
            "insight": "💡 Health score keuangan sebaiknya pakai rasio pengeluaran per "
                       "kategori vs budget yang ditetapkan user, bukan total income vs expense."
        },
        {
            "no": "BQ6", "icon": "📊",
            "title": "Frekuensi vs Nominal per Kategori",
            "color": "#22d3ee",
            "finding": "Makan & Minum: frekuensi tertinggi | Tagihan: nominal/transaksi tertinggi",
            "detail": "Tagihan rata-rata Rp 109.262/transaksi (kost, bensin, langganan). "
                      "Lain-lain sering terjadi tapi nominalnya kecil (Rp 8.799/transaksi).",
            "insight": "💡 Model klasifikasi perlu memahami bahwa tagihan besar ≠ boros, "
                       "sedangkan makan & minum yang perlu dipantau secara konsisten."
        },
        {
            "no": "BQ7", "icon": "🎯",
            "title": "Pola Data Asli (Persona 1 Real)",
            "color": "#4ade80",
            "finding": "Rata-rata Rp 1.564.815/bulan | Tertinggi: September",
            "detail": "Lain-lain & Makan & Minum mendominasi (masing-masing 232 transaksi). "
                      "Pengeluaran kecil yang sangat sering = karakteristik utama keuangan mahasiswa.",
            "insight": "💡 Kesimpulan: Mahasiswa Indonesia rata-rata habiskan Rp 1,5–1,6jt/bulan "
                       "dengan pola dominan: makan harian + tagihan rutin bulanan."
        },
    ]

    for i in range(0, len(BQS), 2):
        cols = st.columns(2)
        for j, col in enumerate(cols):
            if i + j < len(BQS):
                ...
                with col:
                    st.markdown(f"""
                    <div style='background:#131929;border:1px solid #1f2937;border-left:4px solid {bq["color"]};
                    border-radius:12px;padding:16px 18px;margin-bottom:12px;height:100%;'>
                        <div style='display:flex;align-items:center;gap:8px;margin-bottom:8px;'>
                            <span style='font-size:18px;'>{bq["icon"]}</span>
                            <span style='font-size:11px;font-weight:600;color:{bq["color"]};
                            letter-spacing:.06em;text-transform:uppercase;'>{bq["no"]}</span>
                        </div>
                        <div style='font-size:14px;font-weight:600;color:#f1f5f9;margin-bottom:6px;'>
                            {bq["title"]}
                        </div>
                        <div style='font-size:13px;color:#4ade80;font-weight:500;margin-bottom:8px;'>
                            📍 {bq["finding"]}
                        </div>
                        <div style='font-size:12px;color:#94a3b8;line-height:1.6;margin-bottom:10px;'>
                            {bq["detail"]}
                        </div>
                        <div style='background:#0f2d1f;border-radius:8px;padding:8px 12px;
    ...
    # ── Persona Comparison ──
    st.markdown("### 👥 Perbandingan Antar Persona")

    PERSONAS_INFO = [
        {"name":"Persona 1 — Si Hemat","sub":"Data Asli · Jul–Des 2025",
         "budget":"Rp 1,5–1,6jt/bulan","color":"#1D9E75",
         "icon":"🟢","char":"Bawa bekal, hemat konsisten, prioritas tabungan"},
        {"name":"Persona 2 — Agak Boros","sub":"Dummy · Jan–Des 2024",
         "budget":"~Rp 4,7jt/bulan","color":"#3b82f6",
         "icon":"🔵","char":"GoFood, GrabFood, belanja online, hiburan rutin"},
        {"name":"Persona 3 — Hemat Banget","sub":"Anak Perantau · Jan–Des 2024",
         "budget":"~Rp 1jt/bulan","color":"#a78bfa",
         "icon":"🟣","char":"Warteg, masak sendiri, kost murah, hampir tanpa hiburan"},
        {"name":"Persona 4 — Anak Nongkrong","sub":"Dummy · Jan–Des 2024",
         "budget":"~Rp 5,6jt/bulan","color":"#f59e0b",
         "icon":"🟡","char":"Starbucks, Sushi, Cafe, hiburan sangat tinggi"},
    ]

    p_cols = st.columns(4)
    for col, p in zip(p_cols, PERSONAS_INFO):
        with col:
            st.markdown(f"""
            <div style='background:#131929;border:1px solid #1f2937;border-top:3px solid {p["color"]};
            border-radius:12px;padding:16px;text-align:center;'>
                ...
                <div style='font-size:13px;font-weight:600;color:#f1f5f9;margin-bottom:2px;'>
                    {p["name"]}</div>
                <div style='font-size:11px;color:#64748b;margin-bottom:10px;'>{p["sub"]}</div>
                <div style='font-size:16px;font-weight:700;color:{p["color"]};margin-bottom:8px;'>
                    {p["budget"]}</div>
                <div style='font-size:11px;color:#94a3b8;line-height:1.5;'>{p["char"]}</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    # ── Rekomendasi ──
    st.markdown("### 🚀 Rekomendasi untuk Fitur Budgetly")

    recs = [
        ("🔔", "Smart Alert Harian",
         "Kirim notifikasi jika pengeluaran harian > Rp 200.000 (hari normal) "
         "atau > Rp 500.000 (indikator pengeluaran tidak biasa)."),
        ("📅", "Mode Waspada Awal Semester",
         "Aktifkan otomatis di Juli & September — periode dengan lonjakan "
         "pengeluaran tertinggi berdasarkan pola historis mahasiswa."),
        ("🍽️", "Budget Tracker Makan",
         "Buat kategori Makan & Minum sebagai fitur utama karena frekuensinya "
         "2.588x/tahun. Target realistis: Rp 25.000–35.000/hari."),
        ("💰", "Health Score Keuangan",
         "Hitung berdasarkan rasio pengeluaran per kategori vs budget personal, "
         "bukan perbandingan total income vs expense yang misleading."),
        ("📊", "Laporan Persona",
         "Tampilkan profil pengguna (mirip persona mana) berdasarkan pola "
         "belanja untuk memberikan saran yang lebih personal."),
        ("⚠️", "Deteksi Pengeluaran Abnormal",
         "Flagging otomatis jika ada transaksi Tagihan > Rp 1jt dalam sehari "
         "— kemungkinan bayar kost + tagihan lain bersamaan."),
    ]
    ...
    for i in range(0, len(recs), 3):
        ...
        for j, col in enumerate(r_cols):
            if i + j < len(recs):
                icon, title, desc = recs[i + j]
                with col:
                    st.markdown(f"""
                    <div style='background:#131929;border:1px solid #1f2937;border-radius:12px;
    ...
    # ── Kesimpulan Akhir ──
    st.markdown("""
    <div style='background:linear-gradient(135deg,#0f2d1f,#132235);border:1px solid #2a3550;
    border-radius:16px;padding:24px;'>
        <div style='font-size:14px;font-weight:700;color:#4ade80;margin-bottom:12px;'>
            📝 Kesimpulan Akhir
        </div>
        <div style='font-size:13px;color:#cbd5e1;line-height:1.9;'>
            Analisis keuangan mahasiswa Indonesia menunjukkan bahwa <b style='color:#f1f5f9;'>
            pola pengeluaran sangat dipengaruhi oleh kalender akademik</b> dan gaya hidup.
            Rata-rata mahasiswa menghabiskan <b style='color:#4ade80;'>Rp 1,5–5,6 juta/bulan</b>
            tergantung persona, dengan <b style='color:#4ade80;'>Makan & Minum</b> sebagai
            kategori dominan secara frekuensi dan <b style='color:#4ade80;'>Tagihan</b> sebagai
            kategori dominan secara nominal per transaksi.<br><br>
            Temuan ini menjadi fondasi fitur <b style='color:#f1f5f9;'>Budgetly</b> dalam
            memberikan peringatan pengeluaran yang cerdas, personal, dan kontekstual — sesuai
            dengan kebutuhan nyata mahasiswa Indonesia.
        </div>
    </div>
    """, unsafe_allow_html=True)
st.markdown("---")
